...
from vb_model_ft import MAMI_vb_binary_model_ft
...
path_tl_model = os.path.join("model_tl", "MAMI_vb_binary_model_tl_lvis.model")
checkpoint_dir = "checkpoints_vb_binary_model_ft/"
...
def train_model(device, n_epochs, lr, step_size, train_dataloader, val_dataloader):
    ...
    for epoch in range(0, n_epochs):  # 5 epochs at maximum
        ...
        epoch_name = f"MAMI_vb_binary_model_ft_{epoch}.model"
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--frozen_layers",
        type=int,
        help="Number of frozen layers in VisualBERT during training",
    ...
    lr = args.lr
    gamma = args.gamma
    n_frozen_layers = args.frozen_layers
    ...
    model = MAMI_vb_binary_model_ft(path_tl_model=path_tl_model, device=device, n_frozen_layers=n_frozen_layers)
    model.to(device)
    model.train()
    ...
    print(f"Model trainable parameters: {model_trainable_params}")
