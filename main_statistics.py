...
def calculate_feats_patches(model, x_image):
        ...
        with torch.no_grad():
            ...
            for i in range(len(proposals)):
                ...
                n_boxes += len(pred_instances[0].get("pred_boxes"))
                for b in pred_instances[0].get("pred_boxes"):
                    total_area += (b[2].item() - b[0].item()) * (b[3].item() - b[1].item())
                    relative_area += (b[2].item() - b[0].item()) * (b[3].item() - b[1].item()) / (pred_instances[0]._image_size[0] * pred_instances[0]._image_size[1])

        return n_boxes, total_area, relative_area
...
print("LVIS classes:", len(lvis_classes))

#################################################################################################

print("Loading train dataloader..")
train_dataloader = torch.load("dataloaders/train_vb_binary_dataloader_light.bkp")
print("Loading val dataloader..")
val_dataloader = torch.load("dataloaders/val_vb_binary_dataloader_light.bkp")
print("Loading test dataloader..")
test_dataloader = torch.load("dataloaders/test_vb_binary_dataloader_light.bkp")

cfg_maskr_lvis = "LVISv0.5-InstanceSegmentation/mask_rcnn_R_101_FPN_1x.yaml"
...
maskr = build_model(cfg)
checkpointer = DetectionCheckpointer(maskr)  # load weights
checkpointer.load(cfg.MODEL.WEIGHTS)
maskr.eval()  # eval mode
...
for i, data in enumerate(tqdm(train_dataloader), 0):
    texts, images, label = data
    c, ta, ra = calculate_feats_patches(maskr, images)
    object_count += c
    total_area += ta
    relative_area += ra
    ...
    for path in images:
        ...
        image = torch.as_tensor(image.astype("float32").transpose(2, 0, 1))
        inputs.append({"image": image, "height": height, "width": width})
    predicted = maskr(inputs)
    for e,f in zip(predicted, images):
        #print(f, " - ", e["instances"].get("pred_classes"))
        for v in e["instances"].get("pred_classes"):
            classes[v.item()] = classes[v.item()] + 1

print("TRAIN set COCO pretraining")
print("\tDetected objects:", str(object_count))
print("\tDetected objects per image:", str(object_count/7500))
print("\tTotal selected area:", str(total_area))
print("\tAverage % of image selected:", str(relative_area/7500))
for i in range(len(classes)):
    if classes[i] > 500:
        print("index:",i, " - counter:", classes[i], "  - percentage:", classes[i]/7500.0)


cfg_path = cfg_maskr_lvis
cfg = get_cfg()
cfg.merge_from_file(model_zoo.get_config_file(cfg_path))
cfg.MODEL.ROI_HEADS.SCORE_THRESH_TEST = 0.5  # ROI HEADS SCORE THRESHOLD
cfg.MODEL.WEIGHTS = model_zoo.get_checkpoint_url(cfg_path)
...
print("TRAIN set LVIS pretraining")
print("\tDetected objects:", str(object_count))
print("\tDetected objects per image:", str(object_count/7500))
print("\tTotal selected area:", str(total_area))
print("\tAverage % of image selected:", str(relative_area/7500))
for i in range(len(classes)):
    if classes[i] > 100:
        print("index:", lvis_classes[i], " - counter:", classes[i], "  - percentage:", classes[i] / 7500.0)
...
print("VALIDATION set COCO pretraining")
print("\tDetected objects:", str(object_count))
print("\tDetected objects per image:", str(object_count/2500))
print("\tTotal selected area:", str(total_area))
print("\tAverage % of image selected:", str(relative_area/2500))
for i in range(len(classes)):
    if classes[i] > 100:
        print("index:", i, " - counter:", classes[i], "  - percentage:", classes[i] / 2500.0)
...
print("VALIDATION set LVIS pretraining")
print("\tDetected objects:", str(object_count))
print("\tDetected objects per image:", str(object_count/2500))
print("\tTotal selected area:", str(total_area))
print("\tAverage % of image selected:", str(relative_area/2500))
for i in range(len(classes)):
    if classes[i] > 100:
        print("index:", lvis_classes[i], " - counter:", classes[i], "  - percentage:", classes[i] / 2500.0)
...
print("TEST set COCO pretraining")
print("\tDetected objects:", str(object_count))
print("\tDetected objects per image:", str(object_count/1000))
print("\tTotal selected area:", str(total_area))
print("\tAverage % of image selected:", str(relative_area/1000))
for i in range(len(classes)):
    if classes[i] > 50:
        print("index:", i, " - counter:", classes[i], "  - percentage:", classes[i] / 1000.0)
...
print("TEST set LVIS pretraining")
print("\tDetected objects:", str(object_count))
print("\tDetected objects per image:", str(object_count/1000))
print("\tTotal selected area:", str(total_area))
print("\tAverage % of image selected:", str(relative_area/1000))
for i in range(len(classes)):
    if classes[i] > 50:
        print("index:", lvis_classes[i], " - counter:", classes[i], "  - percentage:", classes[i] / 1000.0)
